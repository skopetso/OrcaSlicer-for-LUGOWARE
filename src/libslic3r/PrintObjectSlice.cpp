...
void PrintObject::slice_volumes()
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Slicing volumes - make_slices in parallel - begin";
    {
        ...
        const bool   xy_contour_layer_step_start_on = m_config.xy_contour_compensation_layer_step_start_on.value;
	    ...
	    tbb::parallel_for(
	        tbb::blocked_range<size_t>(0, m_layers.size()),
			[this, xy_hole_scaled, xy_contour_scaled, xy_hole_layer_step, xy_hole_layer_step_thickness, xy_hole_layer_step_start_on, xy_contour_layer_step, xy_contour_layer_step_thickness, xy_contour_layer_step_start_on, elephant_foot_compensation_scaled, &lslices_elfoot_uncompensated](const tbb::blocked_range<size_t>& range) {
	            for (size_t layer_id = range.begin(); layer_id < range.end(); ++ layer_id) {
	                m_print->throw_if_canceled();
	                ...
	                auto should_apply_step = [](int step, int thickness, bool start_on, size_t lid) -> bool {
	                ...
	                if (layer->m_regions.size() == 1) {
                        ...
                        if (elfoot > 0) {
                            ...
                            if (effective_xy_contour_scaled > 0 || effective_xy_hole_scaled > 0) {
                                expolygons_to_compensate = _shrink_contour_holes(std::max(0.f, effective_xy_contour_scaled),
                                                                   std::max(0.f, effective_xy_hole_scaled),
                                                                   expolygons_to_compensate);
                            }
                            if (effective_xy_contour_scaled < 0 || effective_xy_hole_scaled < 0) {
                                expolygons_to_compensate = _shrink_contour_holes(std::min(0.f, effective_xy_contour_scaled),
                                                                   std::min(0.f, effective_xy_hole_scaled),
                                                                   expolygons_to_compensate);
                            }
                            ...
                            if (effective_xy_contour_scaled != 0.0f || effective_xy_hole_scaled != 0.0f) {
                                ExPolygons expolygons = to_expolygons(std::move(layerm->slices.surfaces));
                                if (effective_xy_contour_scaled > 0 || effective_xy_hole_scaled > 0) {
                                    expolygons = _shrink_contour_holes(std::max(0.f, effective_xy_contour_scaled),
                                                                       std::max(0.f, effective_xy_hole_scaled),
                                                                       expolygons);
                                }
                                if (effective_xy_contour_scaled < 0 || effective_xy_hole_scaled < 0) {
                                    expolygons = _shrink_contour_holes(std::min(0.f, effective_xy_contour_scaled),
                                                                       std::min(0.f, effective_xy_hole_scaled),
                                                                       expolygons);
                                }
                        ...
                        float max_growth = std::max(effective_xy_hole_scaled, effective_xy_contour_scaled);
                        float min_growth = std::min(effective_xy_hole_scaled, effective_xy_contour_scaled);
                        ...
                        if (max_growth > 0) {
                            ...
                            merged_poly_for_holes_growing = _shrink_contour_holes(std::max(0.f, effective_xy_contour_scaled),
                                                                                  std::max(0.f, effective_xy_hole_scaled),
                        ...
                        if (min_growth < 0.f || elfoot > 0.f) {
                            ...
                            if (min_growth < 0.0f)
                                trimming = _shrink_contour_holes(std::min(0.f, effective_xy_contour_scaled),
                                                                 std::min(0.f, effective_xy_hole_scaled),
                                                                 trimming);
                            //BBS: trim surfaces
                            for (size_t region_id = 0; region_id < layer->regions().size(); ++region_id) {
